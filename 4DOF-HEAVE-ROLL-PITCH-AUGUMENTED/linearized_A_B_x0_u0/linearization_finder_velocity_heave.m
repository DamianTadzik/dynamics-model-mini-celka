...
load("trim_grid_zW_xWdot.mat");

Nz = numel(zWGrid_m);
Nv = numel(xWdotGrid_ms);

linearizations(Nz,Nv) = struct;
...
for i = 1:Nz
    for j = 1:Nv
        ...
        linearizations(i,j).zW     = trims(i,j).zW;
...
save("linearization_grid_zW_xWdot.mat", ...
    "linearizations", ...
    "zWGrid_m", ...
    "xWdotGrid_ms");
...
maxRealEig       = nan(Nz,Nv);
minRealEig       = nan(Nz,Nv);

ctrbRank         = nan(Nz,Nv);
sigmaMinCtrb     = nan(Nz,Nv);
...
%
% Interpretation:
%   max Re(lambda) < 0  -> locally asymptotically stable
%   max Re(lambda) > 0  -> locally unstable
%   max Re(lambda) ~ 0  -> marginal mode / integrator

figure;

imagesc(xWdotGrid_ms, zWGrid_m, maxRealEig);
set(gca,'YDir','normal');
colorbar;
...
%
% Shows how fast the fastest stable mode changes across the operating
% region.

figure;

imagesc(xWdotGrid_ms, zWGrid_m, minRealEig);
set(gca,'YDir','normal');
colorbar;
...
imagesc(xWdotGrid_ms, zWGrid_m, ctrbRank);
set(gca,'YDir','normal');
colorbar;
...
%
% A very small value means that the system may still be mathematically
% controllable, but some state directions are weakly controllable.
%
% log10 is used because the singular values may span many orders of
% magnitude.

figure;

imagesc(xWdotGrid_ms, zWGrid_m, log10(sigmaMinCtrb));
set(gca,'YDir','normal');
colorbar;
...
%
% Gives a simple overview of how the magnitude of the system dynamics
% changes across the operating region.

figure;

imagesc(xWdotGrid_ms, zWGrid_m, normA);
set(gca,'YDir','normal');
colorbar;
...
%
% Gives a simple overview of how the control effectiveness changes across
% the operating region.

figure;

imagesc(xWdotGrid_ms, zWGrid_m, normB);
set(gca,'YDir','normal');
colorbar;
...
A0 = linearizations(iz0,iv0).A;
B0 = linearizations(iz0,iv0).B;

if isempty(A0) || isempty(B0)
    error('Nominal linearization is empty.');
end

deltaA_rel = nan(Nz,Nv);
deltaB_rel = nan(Nz,Nv);
...
imagesc(xWdotGrid_ms, zWGrid_m, 100*deltaA_rel);
set(gca,'YDir','normal');
colorbar;
...
title(sprintf( ...
    'Relative variation of $A$ from nominal trim ($\\dot{x}_W=%.1f$ m/s, $z_W=%.2f$ m)', ...
    xWdotGrid_ms(iv0), zWGrid_m(iz0)), ...
    'Interpreter','latex');
...
imagesc(xWdotGrid_ms, zWGrid_m, 100*deltaB_rel);
set(gca,'YDir','normal');
colorbar;
...
title(sprintf( ...
    'Relative variation of $B$ from nominal trim ($\\dot{x}_W=%.1f$ m/s, $z_W=%.2f$ m)', ...
    xWdotGrid_ms(iv0), zWGrid_m(iz0)), ...
    'Interpreter','latex');
...
controlEffectiveness = nan(Nz,Nv);
dynamicVariation     = nan(Nz,Nv);
...
figure;
imagesc(xWdotGrid_ms, zWGrid_m, 100*controlEffectiveness);
set(gca,'YDir','normal');
colorbar;

xlabel('Forward velocity $\dot{x}_W$ [m/s]', 'Interpreter','latex');
ylabel('Heave position $z_W$ [m]', 'Interpreter','latex');
...
figure;
imagesc(xWdotGrid_ms, zWGrid_m, 100*dynamicVariation);
set(gca,'YDir','normal');
colorbar;
